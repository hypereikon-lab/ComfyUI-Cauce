...
class TwoSidedGuideWindowTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_default_plan_uses_two_22_frame_guides(self):
        plan = plan_two_sided_guide_window(60, 70)
        ...
        self.assertEqual(plan["generated_range"], [22, 102])
        self.assertEqual(plan["generated_frames"], 80)
        self.assertEqual(plan["assembled_frames"], 210)

    def test_plan_is_deterministic(self):
        first = plan_two_sided_guide_window(60, 70, guide_frames=39, target_frames=141)
        second = plan_two_sided_guide_window(60, 70, guide_frames=39, target_frames=141)
    ...
    def test_assembles_only_generated_range(self):
        plan = plan_two_sided_guide_window(60, 70)
        ...
        np.testing.assert_array_equal(accepted, generated[22:102])
    ...
    def test_rejects_invalid_geometry_and_counts(self):
        with self.assertRaisesRegex(ValueError, "24 fps"):
            plan_two_sided_guide_window(60, 70, fps=23.976)
        with self.assertRaisesRegex(ValueError, "guide_frames"):
            plan_two_sided_guide_window(60, 70, guide_frames=21)
        with self.assertRaisesRegex(ValueError, "target_frames"):
            plan_two_sided_guide_window(60, 70, target_frames=125)
        with self.assertRaisesRegex(ValueError, "at least"):
            plan_two_sided_guide_window(20, 70)
        with self.assertRaisesRegex(ValueError, "non-empty"):
            plan_two_sided_guide_window(73, 73, guide_frames=73, target_frames=124)

    def test_rejects_tampered_plan_and_wrong_generated_length(self):
        plan = plan_two_sided_guide_window(60, 70)
        tampered = dict(plan, target_frames=141)
        with self.assertRaisesRegex(ValueError, "hash"):
            extract_two_sided_window_guides(self.left, self.right, tampered)
